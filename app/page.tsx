...
import { Calendar, MapPin, Ticket, Trophy, ArrowRight, Sparkles, ExternalLink, Briefcase, Music, Zap, Palette, Hash } from "lucide-react";
import { motion } from "framer-motion";
import StarBackground from "@/components/sections/StarBackground";

// --- LINKS & DATA ---
const MAP_LINK = "https://www.google.com/maps/search/?api=1&query=St.+Joseph's+Degree+and+PG+College&query_place_id=ChIJW4o8H9mZyzsRFmzBpbTt4Eo";
    ...
    <TooltipProvider delayDuration={0}>
      <main className="min-h-screen relative overflow-x-hidden bg-[#0f0518]">
        ...
        <section className="relative pt-16 sm:pt-20 md:pt-32 pb-12 sm:pb-16 md:pb-20 px-4 sm:px-6 flex flex-col items-center text-center z-10 min-h-screen justify-center">
          ...
          >
            {/* College Header */}
            <motion.div variants={fadeInUp} className="mb-4 sm:mb-6 space-y-1 sm:space-y-2">
              ...
              <p className="text-[0.65rem] sm:text-xs md:text-xs text-purple-400 font-mono">Department of Business Management Presents</p>
            </motion.div>

            {/* Title */}
            <motion.div variants={fadeInUp} className="relative mb-6 sm:mb-8 group">
            ...
            <motion.h2 variants={fadeInUp} className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-serif text-white/90 mt-[-10px] sm:mt-[-15px] md:mt-[-20px] mb-6 sm:mb-8">
              2026
            </motion.h2>

            <motion.p variants={fadeInUp} className="text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl text-purple-200 mb-8 sm:mb-10 md:mb-12 font-light italic px-2">
              "A Platform for Ideas and Innovation"
            </motion.p>

            {/* BADGES */}
            <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row flex-wrap justify-center gap-3 sm:gap-4 mb-10 sm:mb-12 w-full px-2">
               <Tooltip>
                 <TooltipTrigger asChild>
                   <a href={CALENDAR_LINK} target="_blank" rel="noreferrer" className="w-full sm:w-auto">
               ...
               <Tooltip>
                 <TooltipTrigger asChild>
                   <a href={MAP_LINK} target="_blank" rel="noreferrer" className="w-full sm:w-auto">
            ...
            <motion.div variants={fadeInUp} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              <Link href="../register" className="block w-full sm:w-auto px-2">
                <Button className="relative overflow-hidden bg-white text-black hover:text-white hover:bg-transparent border border-white px-6 sm:px-10 py-5 sm:py-7 text-base sm:text-lg rounded-full transition-all duration-300 group w-full sm:w-auto min-h-[44px] sm:min-h-auto">
        ...
        <section className="px-4 sm:px-6 py-12 sm:py-16 md:py-20 max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 relative z-10">
          <TiltCard delay={0.1} borderColor="border-pink-500/50">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3 sm:gap-4 mb-4">
              <div className="p-2 sm:p-3 bg-pink-500/20 rounded-full"><Ticket className="w-6 sm:w-8 h-6 sm:h-8 text-pink-400" /></div>
              <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-white">Entry: ₹100</h3>
            </div>
            <p className="text-sm sm:text-base md:text-lg text-purple-200 mb-4">Includes 1 Game for <span className="text-pink-400 font-bold">FREE!</span></p>
            <div className="h-px w-full bg-gradient-to-r from-pink-500/50 to-transparent my-4"></div>
            <p className="text-xs sm:text-sm text-gray-400 font-mono">* Extra games: ₹50 each.</p>
          </TiltCard>

          <TiltCard delay={0.3} borderColor="border-yellow-500/50">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3 sm:gap-4 mb-4">
              <div className="p-2 sm:p-3 bg-yellow-500/20 rounded-full"><Trophy className="w-6 sm:w-8 h-6 sm:h-8 text-yellow-400" /></div>
              <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-white">Prize Pool</h3>
            </div>
            <p className="text-sm sm:text-base md:text-lg text-purple-200 mb-2">Total cash prizes worth</p>
        ...
        <section className="py-16 sm:py-20 md:py-24 px-4 sm:px-6 max-w-7xl mx-auto relative z-10">
          ...
          >
            <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-serif text-white mb-3 sm:mb-4">Our Events</h2>
            <p className="text-sm sm:text-base md:text-lg text-purple-300 max-w-2xl mx-auto px-4">
              Six unique challenges designed to test your business acumen, creativity, and technical skills.
            </p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 lg:gap-24">
            ...
            {/* Formal Column */}
            <EventColumn title="Formal Events" colorClass="text-pink-400" borderColor="border-pink-500/30">
              ...
              <EventItem href="/events/brand-revival" icon={Sparkles} title="Brand Revival" desc="Rebrand a failing company." color="group-hover:text-pink-400" border="group-hover:border-pink-500/50" />
              <EventItem href="/events/tech-trek" icon={Zap} title="Tech Trek" desc="The ultimate IT & Business quiz." color="group-hover:text-pink-400" border="group-hover:border-pink-500/50" />
            </EventColumn>

            {/* Informal Column */}
            <EventColumn title="Informal Events" colorClass="text-purple-400" borderColor="border-purple-500/30">
              <EventItem href="/events/meme-market" icon={Hash} title="Meme Market" desc="Viral marketing through humor." color="group-hover:text-purple-400" border="group-hover:border-purple-500/50" />
              <EventItem href="/events/tune-trap" icon={Music} title="Tune Trap" desc="Guess the beat, win the heat." color="group-hover:text-purple-400" border="group-hover:border-purple-500/50" />
              <EventItem href="/events/corporate-canvas" icon={Palette} title="Corporate Canvas" desc="Design the future of ads." color="group-hover:text-purple-400" border="group-hover:border-purple-500/50" />
            </EventColumn>
          </div>
        </section>
        ...
        <footer className="relative z-10 bg-black/40 backdrop-blur-xl border-t border-white/10 py-8 sm:py-10 md:py-12 text-center mt-12 sm:mt-16 md:mt-20 px-4">
           <p className="text-purple-300/60 text-xs sm:text-sm">© 2026 Innovesta. Designed by IT Dept.</p>
        </footer>
      </main>
    </TooltipProvider>
  );
...
const EventColumn = ({ title, colorClass, borderColor, children }: any) => (
  <div className="space-y-4 sm:space-y-6">
    <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8 border-b border-white/10 pb-3 sm:pb-4">
      <h3 className={`text-2xl sm:text-3xl font-bold ${colorClass} font-serif tracking-wide`}>{title}</h3>
      <div className={`h-1 flex-1 bg-gradient-to-r from-transparent to-transparent ${title.includes("Formal") ? "via-pink-500/50" : "via-purple-500/50"} rounded-full`}></div>
    </div>
...
const EventItem = ({ title, desc, icon: Icon, color, border, href = "#" }: any) => (
  <Link href={href} className="block">
    ...
    >
      ...
      <div className={`absolute inset-0 opacity-0 group-hover:opacity-10 bg-gradient-to-r ${title.includes("Venture") ? "from-pink-500" : "from-purple-500"} to-transparent transition-opacity duration-500`}></div>
      ...
      <div className="relative flex items-center justify-between z-10 w-full">
        <div className="flex items-start sm:items-center gap-3 sm:gap-4 md:gap-5 flex-1 min-w-0">
          <div className={`p-2 sm:p-3 rounded-lg sm:rounded-xl bg-white/5 group-hover:scale-110 transition-transform duration-300 ${color} group-hover:bg-white/10 shrink-0`}>
            <Icon className="w-5 sm:w-6 h-5 sm:h-6" />
          </div>
          <div className="min-w-0 flex-1">
            <h4 className={`text-base sm:text-lg md:text-xl font-bold text-white ${color} transition-colors truncate sm:whitespace-normal`}>{title}</h4>
            <p className="text-xs sm:text-sm text-gray-400 group-hover:text-gray-300 mt-0.5 sm:mt-1 line-clamp-1 sm:line-clamp-none">{desc}</p>
          </div>
        </div>
